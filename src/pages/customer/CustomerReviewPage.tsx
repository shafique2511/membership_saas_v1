...
export function CustomerReviewPage() {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true)
    try {
      setBusiness(await getPublicBusinessBySlug(businessSlug))
    } catch (error) {
          ...
          <CardContent className="p-5">
            {submitted ? (
              ...
              <form className="space-y-4" onSubmit={(event) => void handleSubmit(event)}>
                <Field label="Rating" description="Choose how satisfied you were with the visit.">
                    ...
                    {[1, 2, 3, 4, 5].map((value) => (
                      <button
                        ...
                        className={cn(
                          ...
                          value <= rating ? 'text-amber-500' : 'text-slate-300 hover:text-amber-400',
                        )}
                        aria-label={`${value} star rating`}
                      >
                        <Star className={cn('h-8 w-8', value <= rating ? 'fill-current' : '')} />
                      </button>
                    ))}
                  </div>
                </Field>
